...
#define TIME_BETWEEN_MEASURE (2) // Time between measurements in seconds
...
void AD5940BIAStructInit(void)
{
  ...
  pBIACfg->BiaODR = 20;         /* ODR(Sample Rate) 20Hz */ //fréquence à laquelle le séquenceur se déclenche (20mesures/s) == Nbre d'interrupt recu
  ...
  while(1)
  {
    ...
    if(AD5940_GetMCUIntFlag())
    {
      ...
      temp = APPBUFF_SIZE;
			...
			//lit les données de la FIFO du AD5940 et stocke les résultats dans AppBuff
      ...
      if(IntCount >= 1) //On arrête le sequenceur après 1 mesure (on fait donc pas 20mesures/s)
      {
        IntCount = 0;
        AppBIACtrl(BIACTRL_SHUTDOWN, 0);
      }
    }    
		AD5940_Delay10us(100000); /* 1 second delay */
		count++;
    if(count >= TIME_BETWEEN_MEASURE)
    {
