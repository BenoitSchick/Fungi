...
#define TEST_1		RPI_V2_GPIO_P1_35
#define TEST_2		RPI_V2_GPIO_P1_37
...
{   	
    ...
    bcm2835_gpio_set_pud(TEST_1, BCM2835_GPIO_PUD_OFF);
    bcm2835_gpio_set_pud(TEST_2, BCM2835_GPIO_PUD_OFF);
    ...
    bcm2835_gpio_fsel(TEST_1, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(TEST_2, BCM2835_GPIO_FSEL_OUTP);
    ...
    bcm2835_gpio_set(TEST_1);
    bcm2835_gpio_set(TEST_2);
	...
	if (ad7770_setup(&device, init_param) != SUCCESS) return 0;
		...
		if(strlen(myfile->d_name) > 1) printf("USB detected: %s\n", myfile->d_name);
		else{
			printf("No USB connected\n");
			return -1;
		}
		sprintf(dir_name, "/media/pi/%s", myfile->d_name);
