...
labels = ["Substrate only", "Substrate with mycelium in colonization phase", "Substrate fully colonized by mycelium"]
...
for i, filename in enumerate(filenames):
    ...
    ax1.semilogx(f, Cs, label=labels[i])#filename.split("/")[-1])
    ax2.semilogx(f, Rs, label=labels[i])#filename.split("/")[-1])
# Labels et grille
...
#ax2.legend()
ax1.legend(loc="lower left", bbox_to_anchor=(0.5, 0.02))
...
# plt.suptitle("ECG Electrodes in H5L11 configuration") 
plt.suptitle("Metal Rod Electrodes in H5L2 configuration")                                                 
